...
//也就是说，桶排序适合排均匀分布的数据。
//
//桶排序将【0，1）区间划分为n个相同大小的子区间，或称为桶。然后，将n个输入数分别放到各个桶中。因为输入数据是均匀、独立地分布在【0，1）区间上，所以一般不会出现很多数落在同一个桶的情况为了得到输出结果，我们先对每个桶中的数进行排序，然后遍历每个桶，按照次序把每个桶中的元素列出来即可。 伪代码和示意图如下所示：
/*
 * 桶排序
