...
public:
    ...
    //删除顶点及其关联边，返回该顶点信息
    Tv remove(size_t u) {
        assert(0 <= u && u < MAX_NODE_NUM && nodes[u]);
    ...
    void insert(size_t u, size_t v, int w) {
        assert(this->edgeNum < MAX_EDGE_NUM && 0 <= u && u < MAX_NODE_NUM && nodes[u] && 0 <= v && v < MAX_NODE_NUM && nodes[v]);
    ...
    void remove(size_t u, size_t v) {
        assert(0 <= u && u < MAX_NODE_NUM && nodes[u] && 0 <= v && v < MAX_NODE_NUM && nodes[v]);
